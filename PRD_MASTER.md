...
This comprehensive Product Requirements Document (PRD) provides complete specifications for migrating the Dofus FM Assistant application from Python to Java 26 with Spring Boot 3.x (latest LTS version). This document serves as the single source of truth for understanding the current Python application, the target Java architecture, and the complete migration strategy.

### Document Purpose

- **For Developers**: Detailed technical specifications for reimplementing every component
- **For Project Managers**: Timeline, phases, and success criteria
- **For QA**: Testing strategy and validation approach
- **For Stakeholders**: Migration rationale and benefits

### What is the Dofus FM Assistant?

The Dofus FM Assistant is a **real-time network packet analyzer** for the Forgemagie (FM) crafting system in the MMORPG game Dofus. It intercepts network packets between the game client and server to provide live assistance with item enhancement calculations, including:

- Real-time weight calculations
- Reliquat (remainder pool) tracking
- FM result classification (SC/SN/EC/EN)
- Live stat change monitoring
...
- 2.4 Packet Parsing - ExchangeObjectMessage
- 2.5 Packet Parsing - ExchangeCraftResultWithObjectDescMessage
...
### Key Technologies
- **Language**: Java 26
- **Framework**: Spring Boot 3.4.x (Latest LTS)
- **GUI**: JavaFX 21+
- **Database**: SQLite with JPA/Hibernate
- **Network**: Pcap4j for packet capture
- **Build**: Maven 3.9+

### Critical Components to Migrate
1. **Packet Parsing** (`dofus_packet.py`) → `fm-network` module
2. **Domain Models** (`item.py`, `rune.py`, `line.py`) → `fm-core/domain`
3. **FM Calculations** (`Item.executeFM()`) → `FMCalculationService`
4. **UI** (`display.py`) → JavaFX UI
5. **Database** (SQLite queries) → Spring Data JPA repositories

### Migration Timeline
- **Estimated Duration**: 6-7 weeks
- **Total Effort**: 27-41 development days
- **Team Size**: 1-2 developers

### Success Metrics
- ✅ 100% functional parity with Python version
- ✅ Reliquat calculations match exactly
- ✅ Packet processing latency < 50ms
- ✅ Unit test coverage > 80%
- ✅ Zero crashes during 100+ FM attempts
...
1. Read **Section 1** for application overview
2. Review **Section 7.7** for timeline
3. Skim **Section 6.1** for tech stack

### For Implementation
1. Follow **Section 7.2** phases sequentially
2. Reference specific sections as needed:
   - Network parsing: **Section 2**
   - Domain models: **Section 3**
   - FM logic: **Section 4**
   - UI: **Section 5**

### For Testing
1. Use test cases in **Section 4.7** (FM logic)
2. Follow validation approach in **Section 7.3**
3. Compare outputs with Python version

### For Architecture Decisions
1. Review **Section 6** for complete architecture
2. Check **Section 6.15** for decision rationale
...
### Code Examples
All code examples are provided in both Python (current) and Java (target) where applicable. Java examples use:
- Spring Boot 3.4.x conventions
- Java 26 syntax (including preview features if beneficial)
- Lombok for boilerplate reduction
- Modern Java idioms (streams, optionals, etc.)

### Terminology
- **FM**: Forgemagie (item enhancement system in Dofus)
- **Reliquat**: Remainder/pool that tracks failed FM weight
- **SC/SN/EC/EN**: Result types (Success Clean, Success Neutral, Échec Clean, Échec Neutral)
- **GID**: Generic Item ID (item type identifier)
- **UID**: Unique Item ID (specific item instance)

### Important Markers
- ⚠️ **CRITICAL**: Essential for correct functionality
- 💡 **TIP**: Best practice or optimization
- 🐛 **BUG**: Potential issue in original code
- ✅ **VALIDATED**: Tested against Python version
...
```
dofus-fm-assistant/
├── COMPLETE_PRD.md              # This document
├── README.md                     # User-facing documentation
├── pom.xml                       # Parent POM
├── database.sqlite               # Game data database
├── fm-core/                      # Core business logic
├── fm-network/                   # Network packet handling
├── fm-ui/                        # JavaFX user interface
├── fm-app/                       # Main application
...
- Review relevant section in this PRD
- Check JavaDoc in code
- Consult Spring Boot 3.x documentation

### For Dofus Protocol Questions
- Analyze packet captures from live game
- Refer to Section 2 for protocol details
- Compare with Python implementation

### For Testing Assistance
- See Section 7.3 (Testing Strategy)
- Review test examples in Section 4.7
- Run comparison tests with Python version

---

**Ready to Begin?**

Start with **Section 1** for a comprehensive overview of the application and migration objectives, then proceed through each section sequentially for complete understanding.

**Estimated Reading Time**: 3-4 hours for complete document

**Estimated Implementation Time**: 6-7 weeks following the phased approach

---

*This PRD is a living document. Update as implementation progresses and new requirements emerge.*
